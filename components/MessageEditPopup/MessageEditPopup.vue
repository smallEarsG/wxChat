...
export default {
  ...
  methods: {
    ...
    open(content = '') {
      try {
        console.log('=== MessageEditPopup.open 被调用 ===');
        console.log('接收到的内容:', content);
        
        this.messageContent = content || '';
        this.showEmojiPicker = false;
        
        console.log('设置的消息内容:', this.messageContent);
        console.log('popup引用:', this.$refs.popup);
        
        if (this.$refs.popup) {
          console.log('正在打开MessageEditPopup弹窗...');
          this.$refs.popup.open('center');
        } else {
          console.error('MessageEditPopup弹窗引用不存在');
          uni.showToast({
            title: '弹窗打开失败',
            icon: 'none'
          });
        ...
        console.error('打开消息编辑弹窗时发生错误:', error);
        uni.showToast({
          title: '弹窗打开失败',
          icon: 'none'
        });
    ...
    closePopup() {
      try {
        ...
        console.error('关闭消息编辑弹窗时发生错误:', error);
      }
    },
