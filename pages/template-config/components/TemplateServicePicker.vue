	...
	<view v-if="visible" class="modal-mask" @click="$emit('close')">
		<view class="modal-panel" @click.stop>
			<view class="modal-header">
				<text class="modal-title">添加服务项</text>
			...
			<scroll-view class="picker-list" scroll-y>
				<view v-if="filteredItems.length === 0" class="empty-tip">没有可添加的服务项</view>
				<view
				...
				>
					<checkbox :checked="selectedIds.includes(item._id)" color="#4A90E2" />
			...
			<view class="modal-footer">
				...
				<button class="footer-btn primary" :disabled="selectedIds.length === 0" @click="$emit('confirm')">
					添加所选服务项{{ selectedIds.length > 0 ? `(${selectedIds.length})` : '' }}
				</button>
			</view>
		</view>
	</view>
