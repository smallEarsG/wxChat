	...
	export default {
		...
		methods: {
			...
			async getMessageList() {
				try {
					console.log('获取对话列表，userId:', this.userId);
					const res = await getConversationsByUser(this.userId,'wxchat')
					console.log('获取对话列表成功:', res);
			...
			addMsgbox() {
				...
				this.$refs.wxChatPopup.open(testData)
			},
			hasHttp(str) {
				// 正则说明：
				// ^ 表示字符串开头
			...
			async onSubmitWx(data) {
				...
				if (data.conversationId) {
					...
					try {
						...
						// 尝试发送完整数据
						console.log('尝试发送完整数据:', temp);
						try {
							const rul2 = await createConversation(temp)
							console.log('完整数据API响应:', rul2);
							if (rul2.code === 200) {
								this.getMessageList()
								uni.showToast({
									title: '添加成功',
								...
								uni.showToast({
									title: rul2.message || '对话添加失败',
									icon: 'none'
								});
							}
						} catch (error2) {
							console.error('完整数据也失败:', error2);
							uni.showToast({
