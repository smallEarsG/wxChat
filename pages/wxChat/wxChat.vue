<template>
	...
	<view class="chat-page">
		...
		<view class="chat-content">
			...
			<WxTf name="name" amount="amount" ></WxTf>
				<view v-if="activeMsgIndex !== -1" class="overlay" @click="closePopupMenu"></view>
				<!-- 聊天内容 -->
				<view v-for="(item, i) in massageList" :key="i">

					<!-- 时间 -->
					<view v-if="item.type == 'tips'" class="msg-time" @longpress="showPopupMenu($event, i)">
						{{item.content}}</view>
					...
					<!-- 收款 -->
					<view v-else-if="item.contentType == 'wxtf'">
						<view class="msg left" v-if="item.location == 0">
							<image class="avatar" :src="guestInfo.avatar || '/static/avatar-other.png'" />
				   ...
				   <!-- 红包 -->
					<view v-else-if="item.contentType == 'redBag'" @click="getRB(i)">
						<view class="msg left" v-if="item.location == 0"  >
							<image class="avatar" :src="guestInfo.avatar || '/static/avatar-other.png'" />
					...
					<view v-else-if="item.contentType == 'yuyin'">
						<view class="msg left " @longpress="showPopupMenu($event, i)" v-if="item.location == 0">
							<image class="avatar" :src="guestInfo.avatar || '/static/avatar-other.png'" />
							<view class="bubble">
								<view class="yuyinBox" :style="{width:(108 + (item.content.time - 1) * 20)+'rpx'}">
									{{item.content.time}}
								</view>
					...
					<view v-if="activeMsgIndex === i" class="popup-menu"
						...
						<!-- <view class="menu-item" @click="copyMessage(item.content)">复制</view> -->
	...
	export default {
		...
		methods: {
			...
			showPopupMenu(e, index) {
				...
				this.popupTop = touch.clientY - 100; // 往上偏移
				this.popupLeft = touch.clientX - 50; // 居中偏移
			},
