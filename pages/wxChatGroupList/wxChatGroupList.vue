...
<script>
import { createGroup, getGroupsByUser, updateGroup, deleteGroup as deleteGroupApi } from '@/api/groups.js'
...
export default {
  ...
  methods: {
          ...
          try {
            ...
            if (res.code === 200 && res.data) {
              ...
              this.groupList = res.data.map(group => ({
                ...
                lastMessageTime: group.last_message_time ? new Date(group.last_message_time).getTime() : new Date(group.created_at).getTime()
              }));
            }
          } catch (apiError) {
            console.error('从后端加载群聊列表失败:', apiError);
          }
        }
      } catch (e) {
        console.error('加载威信群聊列表失败:', e);
      }
    },
      ...
      try {
        if (this.isEdit) {
          ...
          const createData = {
            ...
            avatar: this.currentGroup.avatar || '',
            description: this.currentGroup.description || '',
            type: this.currentGroup.type || 1,
            ownerId: userId  // 群主ID默认为当前用户ID
          };
          ...
          if (res.code === 200 && res.data) {
            ...
            const newGroup = {
              ...
              type: res.data.type,
              createTime: new Date(res.data.created_at).getTime(),
              lastMessageTime: Date.now()
            };
          ...
          if (res.confirm) {
            ...
            try {
              ...
              if (group.id && typeof group.id === 'number') {
                const deleteRes = await deleteGroupApi(group.id);
    ...
    enterGroup(group) {
      ...
      uni.navigateTo({
        url: `/pages/wxChatGroup/wxChatGroup?guestInfo=${encodeURIComponent(JSON.stringify(groupInfo))}`
      });
    },
